...
-- ═══════════════════════════════════════════════════════════════════
-- Adds sales_order_2.bill_alias — an optional per-bill customer name for a
-- split bill. When set it replaces the "Bill N" label everywhere the bill is
-- shown on the POS; null/empty falls back to the numeric label. Cleared when a
-- split group collapses back to a single order after a merge. Display-only; no
-- server-side object reads it.
--
-- Idempotent: ADD COLUMN IF NOT EXISTS makes re-runs a no-op.
